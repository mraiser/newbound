...
pub fn execute(o: DataObject) -> DataObject {
    use std::panic;
    for p in ["repo", "lib", "ctl", "message", "author", "nn_sessionid"] {
        if !o.has(p) {
    ...
    let ax = panic::catch_unwind(panic::AssertUnwindSafe(|| {
        ...
        let arg_2: String = o.get_string("ctl");
        let arg_3: String = o.get_string("message");
        let arg_4: String = o.get_string("author");
        let arg_5: String = o.get_string("nn_sessionid");
        commit_unit(arg_0, arg_1, arg_2, arg_3, arg_4, arg_5)
    }));
    match ax {
        Ok(ax) => {
            let mut result_obj = DataObject::new();

...
use flowlang::appserver::add_event_listener;
...
pub fn execute(o: DataObject) -> DataObject {
    use std::panic;
    for p in ["lib", "ctl", "name", "bot", "event", "cmd", "author", "nn_sessionid"] {
        if !o.has(p) {
    ...
    let ax = panic::catch_unwind(panic::AssertUnwindSafe(|| {
        ...
        let arg_3: String = o.get_string("bot");
        let arg_4: String = o.get_string("event");
        let arg_5: String = o.get_string("cmd");
        let arg_6: String = o.get_string("author");
        let arg_7: String = o.get_string("nn_sessionid");
        set_event_handler(arg_0, arg_1, arg_2, arg_3, arg_4, arg_5, arg_6, arg_7)
    }));
    match ax {
        Ok(ax) => {
            let mut result_obj = DataObject::new();

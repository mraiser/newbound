...
me.ready = function(){
  ...
  json('../botmanager/read', 'db=runtime&id=peerbot_mypeers', function(result){
    console.log('mypeers');
    console.log(result);
    if (result.data) document.body.mypeers = result.data.list;
    else document.body.mypeers = [];
  });
};
...
connect = function(uuid){
  var addr = $('#li_'+uuid).find('.daddr').text();
  var port = $('#li_'+uuid).find('.dport').text();
...
$(ME).find('.updateallpeersbutton').click(function(){
  $('#updatelist').html('');
  var el = $(ME).find('.updatepopup');
  installControl(el[0], 'metabot', 'popupdialog', function(){
    el.css('display', 'block');
    setTimeout(function(){
      installControl('#updatelist', 'peerbot', 'update_remote_peers', function(api){
        $(ME).find('.updatepopupclose').click(function(){
          el[0].api.close(function(){
            el.empty();
            el.append(el[0].api.oldhtml);
            el.css('display','none');
